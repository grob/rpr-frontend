define([
     ...
     * @type String
     */
    dates.format = function(date, pattern) {
        if (date == undefined) {
            return "";
    ...
    dates.formatDuration = function(millis) {
        var s = Math.floor(millis / 1000);
        return [strings.pad(Math.floor(s / 3600), "0", 2, -1),
            strings.pad(Math.floor((s % 3600) / 60), "0", 2, -1),
            strings.pad(s % 60, "0", 2, -1)].join(":");
    };

    dates.formatTime = function(millis) {
        var s = Math.floor(millis / 1000);
        return [
            strings.pad(Math.floor(s / 3600), "0", 2, -1), ":",
            strings.pad(Math.floor((s % 3600) / 60), "0", 2, -1), ":",
            strings.pad(s % 60, "0", 2, -1), ".",
            strings.pad(millis % 1000, 0, 3, -1)
        ].join("");
    };

    dates.parseDay = function(day) {
        if (typeof(day) !== "string") {
            day = String(day);
        }
        var date = new Date(0);
        date.setFullYear(day.substr(0, 4));
        date.setMonth(day.substr(4, 2) - 1);
        date.setDate(day.substr(6, 2));
        date.setHours(0);
        date.setMinutes(0);
        date.setSeconds(0);
